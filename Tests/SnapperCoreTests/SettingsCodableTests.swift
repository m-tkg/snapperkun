...
final class SettingsStoreTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("SnapperTests-\(UUID().uuidString)", isDirectory: true)
        tempURL = dir.appendingPathComponent("settings.json")
    }
    ...
    func testLoadMissingFileReturnsEmpty() {
        ...
        XCTAssertEqual(store.load(), Settings.empty)
    }
    ...
    func testSaveThenLoadRoundTrips() throws {
        ...
        let settings = Settings(bindings: [
            Binding(
                id: UUID(),
                keyCombo: KeyCombo(keyCode: 125, modifiers: [.command, .shift]),
                specs: [SnapSpec(width: .full, height: .full, horizontal: .left, vertical: .top)]
            )
        ])
