...
private let log = Logger(subsystem: "com.mtkg.snapperkun", category: "kuntraykun")
...
/// NSMenu を読むためメインスレッドから呼ぶこと（呼び出し元はすべてメインスレッド）。
enum KuntraykunMenuExport {
    ...
    private static func nodes(of menu: NSMenu, path: [Int]) -> [Node] {
        ...
        for (index, item) in menu.items.enumerated() {
            ...
            // カスタムビュー項目は転送できない（v4 のスコープ外）。タイトルのみ・操作不可で書き出す。
            if item.view != nil {
