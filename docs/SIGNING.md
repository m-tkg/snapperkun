...
1. システム設定 > プライバシーとセキュリティ > アクセシビリティ で古い Snapperkun を削除
2. 新しい版を起動して再度許可
...
```
`SIGN_IDENTITY` 未指定なら ad-hoc 署名（Hardened Runtime / タイムスタンプはアイデンティティ
指定時のみ付与）。

## 無料の代替（自己署名）

Apple Developer Program を使わない場合、自己署名のコード署名証明書でも
**アクセシビリティ権限の保持だけ**は実現できます（Gatekeeper 警告は残り、配布には不向き）。
